...
class index_basic(stock_basic.stock_basic):

    ord_add = 'asc'

    def __init__(self,api_name = ''):
        super(index_basic, self).__init__(api_name)
        self.ftime_name = 'base_date'
        self.load_file()
        return
    ...
    def api_query(self):
        ...
        df = pd.DataFrame()
        for mk in markets:
            df = df.append(other=self.pro.index_basic(market=mk),ignore_index=True)
        return df

def list_date(ts_code):
    df = i.data.loc[(i.data['ts_code'].astype(str) == ts_code)]
    if len(df)>0:
        res = df['base_date'].astype(int).iloc[0]
        if math.isnan(res):
            res = ''
    else:
        res = ''
